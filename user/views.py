...
class RegisterAPIView(APIView):
    def post(self, request):
        ...
        if s.is_valid() or (user and not user.is_active):
            ...
            # return response
            return JsonResponse({"message": "User created"}, status=HTTP_200_OK)
        elif user and user.is_active:
            return JsonResponse({"message": "User with this phone already registered!"}, status=HTTP_400_BAD_REQUEST)
...
#             return JsonResponse({"message": "Successfully registered!"}, status=HTTP_201_CREATED)
#         return JsonResponse(s.errors)
